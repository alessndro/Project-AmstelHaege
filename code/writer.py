...
def write_progress(number_of_houses, map_number, total_value_map, all_houses, algoritme):
    ''' Schrijft het resultaat van de run op in progress file  '''

    # open file
    with open('resultaten/solutions.csv', 'a', newline='') as file:
        writer = csv.writer(file)

        # noteer alle resultaten in het file 
        writer.writerow([number_of_houses, map_number, total_value_map, algoritme])
        for item in all_houses.values():
            for house in item:
                writer.writerow([house.name, house.size, house.bottom_left, house.top_right])

def write_progress_run(number_of_houses, map_number, total_value_map, algoritme):
    ''' Noteert de resultaten van de huidige run '''
    
    with open('resultaten/progress_run.csv', 'a', newline='') as file:
        writer = csv.writer(file)
...
def read_progress_run():
    ...
    with open('resultaten/progress_run.csv', 'r') as file:
...
def delete_progress_run():
    ...
    with open('resultaten/progress_run.csv', 'w') as file:
